    ...
    public class AlphaVantagAPI
    {
        ...
        public async Task<OutData<CompanyOverviewData>> GetCompanyOverview(string shareCode)
        {
            ...
            try
            {
                ...
                SaveCompanyJson(shareCode, json);
                ...
                {
                    output.StatusList.Add(new Status("F", "Not Parsed data well", 0));
        ...
        private static void SaveCompanyJson(string shareCode, string json)
        {
            try
            {
                string folderName = $@"{Settings.Default.LoggingFolder}\Company";
                if(!Directory.Exists(folderName ))
                {
                    Directory.CreateDirectory(folderName);
                }
                string fileName = $@"{folderName}\{shareCode}_COMP.json";
        ...
        private static void LogJsonValue(string shareCode, string json)
        {
            string folderName = $@"{Settings.Default.LoggingFolder}\ShareValue";
            if (!Directory.Exists(folderName))
            {
                Directory.CreateDirectory(folderName);
            }
            string fileName = $@"{folderName}\{shareCode}.json";
            File.WriteAllText(fileName, json);
        }
        ...
        public async Task<bool> GetShareMarketValue(ShareMarketValueData input,
                                                                         string interval = "15min",
                                                                         string outputSize = "compact",
                                                                         bool isAdjusted = true)
        {
            DateTime currentDate = DateTime.Today;
            OutData<TimeSeriesIntradayData> series = await GetShareIntraday(input.TradeCode, interval, outputSize, isAdjusted);
            decimal lowAmount = 0;
            decimal highAmount = 0;
            if (!UtilityHandler.IsValidStatus(series))
            {
                return false;
            }
            int i = 0;
            foreach (TimeSeriesData data in series.Data.Data)
            {
                ...
                {
                    lowAmount = data.Low;
                    highAmount = data.High;
                    input.CurrentAmnt = data.Close;
                    input.CloseAmnt = data.Close;
                    input.TradeDate = data.TradeDate;
                    currentDate = data.TradeDate.Date;
                    continue;
                }
                if (data.Low < lowAmount)
                {
                    lowAmount = data.Low;
                }
                if (data.High < highAmount)
                {
                    highAmount = data.High;
                }
                if (data.TradeDate.Date != currentDate) { break; }
            }
            input.LowAmnt = lowAmount;
            input.HighAmnt = highAmount;
            return true;
        }

        public async Task<OutData<TimeSeriesIntradayData>> GetShareDailyAdjusted(string shareCode )
        {
            ...
            try
            {
                string query = $"/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol={shareCode}";
                string url = $"{Settings.Default.StockApi}{query}&apikey={Settings.Default.ApiKey}";
                using HttpClient client = UtilityHandler.GetHttpClient();
                Uri requestUri = new Uri(url);
                HttpResponseMessage response = await client.GetAsync(requestUri).ConfigureAwait(false);
                ...
                foreach (string key in jo.Keys)
                {
                    ...
                    {
                        data.Data = ParseDailyAdjusted(value);
                    }
                }
                return output;
            }
        ...
        private List<TimeSeriesData> ParseDailyAdjusted(JToken input)
        {
            ...
            foreach (JProperty prop in input.Children<JProperty>())
            {
                ...
                output.Add(GetDailyAdjustedData(key, value));
            }
            return output;
        }

        private TimeSeriesData GetDailyAdjustedData(string timeData, JToken input)
        {
            _ = DateTime.TryParse(timeData, out DateTime marketTime);
            TimeSeriesData output = new TimeSeriesData()
            {
                TradeDate = marketTime
            };
            foreach (JProperty prop in input.Children<JProperty>())
            {
                string key = prop.Name;
                                                                            ...
                                                                            bool isAdjusted = true)
        {
            ...
            try
            {
                ...
                foreach (string key in jo.Keys)
                {
                    ...
                    if (key.StartsWith(AlphaVantagTagConstants.TAG_TIME_SERIES_DAILY))
                    {
        ...
        private static string GetTagValue(Dictionary<string, string> input, string tag)
        {
        ...
        private static long GetTagValueLong(Dictionary<string, string> input, string tag)
        {
            long output = 0;
            ...
            _ = long.TryParse(s, out output);
            return output;
        }

        private static DateTime GetTagValueDateTime(Dictionary<string, string> input, string tag)
        {
            ...
            _ = DateTime.TryParse(s, out output);
            return output;
        }

        private static decimal GetTagValueDecimal(Dictionary<string, string> input, string tag)
        {
            decimal output = 0;
            string s = GetTagValue(input, tag);
            if (UtilityHandler.IsEmpty(s))
            ...
            _ = decimal.TryParse(s, out output);
            return output;
        }
        ...
        {
            _ = long.TryParse(value, out long output);
            return output;
        }
